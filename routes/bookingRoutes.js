...
router.post("/booking", (req, res) => {
    ...
    if (!name) {
        res.status(400).json({ message: "Se till att namn är ifyllt" });
        return;
    } else if (!email) {
        res.status(400).json({ message: "Se till att e-postadress är ifylld" });
        return;
    } else if (!date) {
    ...
    client.query(`INSERT INTO booking (name, email, date, time, message) VALUES ($1, $2, $3, $4, $5);`, [name, email, date, time, message], (err, results) => {
        ...
        if (err) {
            ...
            res.json({ message: "Din bokning är genomförd", booking });
        }
    });
